...
{
    if (vterm->cursor_x >= 600 / VESA_CHAR_WIDTH || c == '\n') {
        vterm->cursor_x = 0;
        vterm->cursor_y++;
    } else if (vterm->cursor_y >= 400 / VESA_LINE_HEIGHT) {
        vterm->cursor_y = 0;
    } else {
        vterm->buffer[vterm->cursor_y * (600 / VESA_CHAR_WIDTH) + vterm->cursor_x] = c;
        vterm->cursor_x++;
    }
}
...
void vterm_paint(window_t *window)
{
    ...
    for (int i = 0; i < (400 / VESA_LINE_HEIGHT); i++) {
        for (int j = 0; j < (600 / VESA_CHAR_WIDTH); j++) {
            auto cur_char = vterm->buffer[i * (600 / VESA_CHAR_WIDTH) + j];
            if (cur_char != 0) {
...
vterm_t *vterm_new()
{
    ...
    vterm->buffer = (char *)kzalloc((600 / VESA_CHAR_WIDTH) * (400 / VESA_LINE_HEIGHT));
    if (!vterm->buffer) {
        kfree(vterm);
        return nullptr;
    }

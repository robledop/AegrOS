 ...
 */
static uintptr_t* walkpgdir(uintptr_t* pgdir, const void* va, int alloc)
{
    uintptr_t* pgtab;

    uintptr_t* pde = &pgdir[PDX(va)];
    if (*pde & PTE_P)
    {
        pgtab = (uintptr_t*)P2V(PTE_ADDR(*pde));
    }
    else
    {
        if (!alloc || (pgtab = (uintptr_t*)kalloc()) == 0)
            return 0;
 ...
 */
static int mappages(uintptr_t* pgdir, void* va, uint32_t size, uint32_t pa, int perm)
{
    uintptr_t* pte;
...
void switch_uvm(struct proc* p)
{
    ...
    mycpu()->task_state.esp0 = (uint32_t)p->kstack + KSTACKSIZE;
    // setting IOPL=0 in eflags *and* iomb beyond the tss segment limit
    // forbids I/O instructions (e.g., inb and outb) from user space
    mycpu()->task_state.iomb = (uint16_t)0xFFFF;
    ltr(SEG_TSS << 3);
 ...
 */
void inituvm(uintptr_t* pgdir, const char* init, uint32_t sz)
{
    if (sz >= PGSIZE)
    {
 ...
 */
int loaduvm(uintptr_t* pgdir, char* addr, struct inode* ip, uint32_t offset, uint32_t sz)
{
    uint32_t n;
    uintptr_t* pte;
 ...
 */
int allocuvm(uintptr_t* pgdir, uint32_t oldsz, uint32_t newsz)
{
    if (newsz >= KERNBASE)
        return 0;
    if (newsz < oldsz)
    ...
    for (uint32_t a = PGROUNDUP(oldsz); a < newsz; a += PGSIZE)
    {
        char* mem = kalloc();
        if (mem == 0)
        {
 ...
 */
int deallocuvm(uintptr_t* pgdir, uint32_t oldsz, uint32_t newsz)
{
    if (newsz >= oldsz)
        return oldsz;
    ...
    {
        uintptr_t* pte = walkpgdir(pgdir, (char*)a, 0);
        if (!pte)
...
{
    uintptr_t* pte = walkpgdir(pgdir, uva, 0);
    if (pte == 0)
        panic("clearpteu");
    *pte &= ~PTE_U;
}
 ...
 */
uintptr_t* copyuvm(uintptr_t* pgdir, uint32_t sz)
{
    uintptr_t* d;
    uintptr_t* pte;
    uint32_t pa, flags;
 ...
 */
char* uva2ka(uintptr_t* pgdir, char* uva)
{
    uintptr_t* pte = walkpgdir(pgdir, uva, 0);
    if ((*pte & PTE_P) == 0)
        return 0;
 ...
 */
int copyout(uintptr_t* pgdir, uint32_t va, void* p, uint32_t len)
{
    char* buf = (char*)p;
    while (len > 0)
    {
        uint32_t va0 = (uint32_t)PGROUNDDOWN(va);
        char* pa0 = uva2ka(pgdir, (char*)va0);
        if (pa0 == 0)
            return -1;
        uint32_t n = PGSIZE - (va - va0);
        if (n > len)
            n = len;

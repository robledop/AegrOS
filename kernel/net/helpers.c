...
void int_to_ip(u32 ip_addr, char *result[16])
{
    const u32 ip = ntohl(ip_addr);
...
/// @endcode
u16 checksum(void *addr, int count, const int start_sum)
{
    register u32 sum = start_sum;
    u16 *ptr         = addr;
...
char *get_mac_address_string(u8 mac[6])
{

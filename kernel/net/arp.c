...
void arp_cache_add(u8 ip[static 4], u8 mac[static 6])
{
    for (int i = 0; i < ARP_CACHE_SIZE; i++) {
        if (arp_cache[i].ip[0] == 0) {
            memcpy(arp_cache[i].ip, ip, 4);
            memcpy(arp_cache[i].mac, mac, 6);
            arp_cache[i].timestamp = ticks;
            return;
        }
    }
}

void arp_receive_reply(u8 *packet)
{
...
void arp_receive(u8 *packet)
{
    ...
    const u16 opcode             = ntohs(arp->opcode);
    switch (opcode) {
    case ARP_REQUEST:

...
void set_request_dhcp_options(u8 *options, u8 ip[4], u8 server_ip[4])
{
    int offset = 0;
...
int dhcp_options_get_dns_servers(const u8 options[static DHCP_OPTIONS_LEN], u32 dns_servers[static 1],
        ...
        if (option_code == DHCP_OPT_END) {
            ...
            if (option_code == DHCP_OPT_DNS) {
                ...
                for (size_t i = 0; i < num_servers; i++) {
                    u32 dns_server_network_order;
                    memcpy(&dns_server_network_order, &options[offset + i * 4], sizeof(u32));
    ...
    while (offset < DHCP_OPTIONS_LEN) {
        ...
        if (option_code == DHCP_OPT_END) {
            ...
            if (option_code == option) {
                ...
                const u32 subnet_mask = ((u32)options[offset] << 24) | (options[offset + 1] << 16) |
...
void set_discover_dhcp_options(u8 *options)
{
    int offset = 0;
...
void dhcp_receive(u8 *packet)
{
    ...
    const u16 dhcp_message_type = dhcp_packet->options[2];
    ...
    if (dhcp_packet->op == DHCP_OP_OFFER && dhcp_message_type == DHCP_MESSAGE_TYPE_OFFER &&
        ...
        u32 dhcp_server_ip_p = dhcp_options_get_ip_option(dhcp_packet->options, DHCP_OPT_SERVER_ID);
    ...
    // This is the final step in the DHCP process and we are now bound to the IP address
        ...
        u32 subnet_mask_p = dhcp_options_get_ip_option(dhcp_packet->options, DHCP_OPT_SUBNET_MASK);
        ...
        u32 gateway_p = dhcp_options_get_ip_option(dhcp_packet->options, DHCP_OPT_ROUTER);
        ...
        for (size_t i = 0; i < dns_server_count; i++) {
            u32 dns_p = dns_servers[i];
            u8 dns[4];
            memcpy(dns, &dns_p, 4);
            boot_message(WARNING_LEVEL_INFO, "%-13s %d.%d.%d.%d", "DNS server:", dns[0], dns[1], dns[2], dns[3]);
        }
...
void dhcp_send_request(u8 mac[6], u8 ip[4], u8 server_ip[4])
{
    boot_message(WARNING_LEVEL_INFO, "Sending DHCP Request...");
    ...
    u8 udp_checksum_buf[sizeof(struct udp_pseudo_header) + sizeof(struct udp_header) + sizeof(struct dhcp_header)];
...
void dhcp_send_discover(u8 mac[6])
{
    boot_message(WARNING_LEVEL_INFO, "Sending DHCP Discover...");
    struct ether_header ether_header = {
        .dest_host = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF},
        .ether_type = htons(ETHERTYPE_IP),
    };

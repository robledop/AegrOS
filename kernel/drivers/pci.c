 ...
 */
u16 pci_config_read_word(const u8 bus, const u8 slot, const u8 func, const u8 offset)
{
    const u32 lbus  = bus;
    ...
    const u32 address = lbus << 16 | lslot << 11 | lfunc << 8 | (offset & 0xFC) | 0x80000000;
    ...
    tmp = (u16)((inl(PCI_CONFIG_DATA) >> ((offset & 2) * 8)) & 0xFFFF);
    return tmp;
}
 ...
 */
void pci_config_write_word(const u8 bus, const u8 slot, const u8 func, const u8 offset,
                           const u16 data)
{
    const u32 lbus  = bus;
    const u32 lslot = slot;
    ...
    const u32 address = (0x80000000 | (lbus << 16) | (lslot << 11) | (lfunc << 8) | (offset & 0xFC));

    outl(PCI_CONFIG_ADDRESS, address);

    u32 tmp = inl(PCI_CONFIG_DATA);
 ...
 */
const char *pci_find_name(const u8 class, const u8 subclass)
{
 ...
 */
const char *pci_find_vendor(const u16 vendor_id)
{
...
void load_driver(const struct pci_header pci, const u8 bus, const u8 device, const u8 function)
{
    ...
    for (u16 i = 0; i < sizeof(pci_drivers) / sizeof(struct pci_driver); i++) {
        ...
        if (class_match && subclass_match && vendor_match && device_match) {
            ...
            boot_message(WARNING_LEVEL_INFO, "Loading driver for %s", pci_find_name(pci.class, pci.subclass));
 ...
 */
struct pci_header get_pci_data(const u8 bus, const u8 num, const u8 function)
{
    struct pci_header pci_data;
    auto const p = (u16 *)&pci_data;
    for (u8 i = 0; i < 32; i++) {
...
void pci_scan()
{
    boot_message(WARNING_LEVEL_INFO, "Scanning PCI devices...");
...
{
    constexpr u16 command_register_offset = 0x04;
    u16 dev_command_reg = pci_config_read_word(device.bus, device.slot, device.function, command_register_offset);
 ...
 */
u32 pci_get_bar(const struct pci_device dev, const u8 type)
{
    u32 bar = 0;
    for (int i = 0; i < 6; i++) {

...
const char *szPhotometric[] = {
    "WhiteIsZero", "BlackIsZero", "RGB", "Palette Color", "Transparency Mask", "CMYK", "YCbCr", "Unknown"};
...
int ImageInfo(FILE *iHandle, int iFileSize, char *szInfo, int *iDataOff)
{
    ...
    case FILETYPE_OS2BMP:
        ...
        if (cBuf[30] == 1 || cBuf[30] == 2 ||
            cBuf[30] == 4)               // if biCompression is non-zero (2=4bit rle, 1=8bit rle,4=24bit rle)
            iCompression = COMPTYPE_RLE; // windows run-length
        break;
    ...
    case FILETYPE_TIFF:
        ...
        //            sprintf(szOptions, ", Photometric = %s, Planar config = %s", szPhotometric[iPhotoMetric],
        //            szPlanar[iPlanar]); break;
    } // switch
    sprintf(szInfo,
            "// %s, Compression=%s, Size: %d x %d, %d-Bpp\n",
            szType[iFileType],
            szComp[iCompression],
            iWidth,
            iHeight,
